...
console.log('🔍 Test de validation des IDs patient dans les routes d\'accès');
console.log('=' * 60);
...
async function testAccessValidation() {
  try {
    ...
    console.log('\n🔍 ÉTAPE 2: Test avec ID patient valide');
    try {
      const validResponse = await axios.get(`${BASE_URL}/access/history/patient/${patientId}`, { headers });
    ...
    // ÉTAPE 3: Test avec ID patient null (doit retourner 400)
    console.log('\n🚫 ÉTAPE 3: Test avec ID patient null (doit retourner 400)');
    try {
      const nullResponse = await axios.get(`${BASE_URL}/access/history/patient/null`, { headers });
    ...
    // ÉTAPE 4: Test avec ID patient undefined (doit retourner 400)
    console.log('\n🚫 ÉTAPE 4: Test avec ID patient undefined (doit retourner 400)');
    try {
      const undefinedResponse = await axios.get(`${BASE_URL}/access/history/patient/undefined`, { headers });
    ...
    console.log('\n🚫 ÉTAPE 5: Test avec ID patient invalide (doit retourner 400)');
    try {
      const invalidResponse = await axios.get(`${BASE_URL}/access/history/patient/abc`, { headers });
    ...
    console.log('\n🚫 ÉTAPE 6: Test avec ID patient négatif (doit retourner 400)');
    try {
      const negativeResponse = await axios.get(`${BASE_URL}/access/history/patient/-1`, { headers });
...
// Exécuter le test
testAccessValidation().catch(console.error);
